...
describe('utils', () => {
  ...
  describe('parseQuery', () => {
    it('should parse some query strings', () => {
      expect(parseQuery('?utm_source=abc&utm_campaign=flower')).toEqual([
        ['utm_source', 'abc'],
        ['utm_campaign', 'flower'],
      ]);
  ...
  describe('shallowCompare', () => {
    it('should compare objects', () => {
      ...
      expect(
        shallowCompare({ name: 'AB', items: [{ a: 1 }] }, { name: 'AB', items: [{ a: 1 }] }),
      ).toBe(true);

      expect(shallowCompare({ a: 1 }, {})).toBe(false);

      expect(shallowCompare({}, { b: 2 })).toBe(false);
    });

    it('should compare arrays', () => {
      expect(shallowCompare([], [])).toBe(true);

      expect(shallowCompare([0, { a: 1 }], [0, { a: 1 }])).toBe(true);

      expect(shallowCompare([0, { a: 1 }], [0, { b: 1 }])).toBe(false);
    });
